...
import { useManufacturerCard } from '../../hooks';
import { getMachineIcon } from '../../hooks/useMachineCard';
import styles from '../../styles';

const ManufacturerCard = ({ machine, navigation }) => {
  const {
    liveMachine,
    currentRecipe,
    machineProcesses,
    ...
    displayName,
    actions,
  } = useManufacturerCard(machine);

  const openManufacturerScreen = () => {
    ...
    >
      ...
      <View style={styles.rowAlignCenter}>
        ...
        <View style={styles.marginVertical10}>
          <TouchableOpacity
            onPress={openManufacturerScreen}
            activeOpacity={0.7}
          >
            <LinearGradient
            ...
            <View style={styles.headerRow}>
              <View style={styles.selectedNodePill}>
                <Text style={styles.selectedNodePillText}>
                  Manufacturing: {currentProcess.itemName} (Queue: {machineProcesses ? machineProcesses.length : 1})
                </Text>
              </View>
        ...
        <View style={styles.extraInfoContainer}>
          ...
          {currentRecipe && (
            <Text style={styles.recipeInfo}>
              Ready to manufacture: {currentRecipe.name}
            </Text>
          )}
        </View>
      )}
