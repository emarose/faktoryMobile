...
const QuantityStepper = ({ amount, setAmount, maxAmount }) => {
  ...
  const calculateThumbPosition = useMemo(
    () => (value) => {
      ...
      return (
        ((boundedValue - 1) / (safeMaxAmount - 1)) * (sliderWidth - thumbSize)
      );
    },
      ...
      PanResponder.create({
        ...
        onPanResponderMove: (evt, gestureState) => {
          ...
          const position = Math.max(
            0,
            Math.min(sliderWidth - thumbSize, touchX - thumbSize / 2)
          );
    ...
    <View style={{ marginVertical: 16, alignItems: "center" }}>
      ...
      >
        ...
        <View style={{ marginHorizontal: 32, alignItems: "center" }}>
          <Text style={{ color: "#e8f4fd", fontWeight: "bold", fontSize: 14 }}>
            {numericValue} / {safeMaxAmount}
          </Text>
        </View>
