...
export default function useOilExtractorCard(machine) {
  const {
    placedMachines,
    allResourceNodes,
    discoveredNodes,
  ...
  const nodeDepletionData = useMemo(() => {
    ...
    try {
      ...
      console.error("Oil Extractor nodeDepletionData error:", err);
      return null;
    }
  ...
  return {
    ...
    nodeDepletionData,
    status,
    machineColor: getMachineColor(machine.type) || "#D76B00", // Orange for oil extractor
    displayName: `Oil Extractor #${liveMachine.id.split('-')[1] || ''}`,
    actions: {
      handlePauseResume,
      handleDetachNode,
    },
