...
const useRefineryCard = (machine) => {
  ...
  const status = useMemo(() => {
    ...
    if (isProcessing && currentProcess) {
      if (currentProcess.status === "paused") {
        ...
        statusText = `Refining: ${currentProcess.itemName}`;
        statusColor = "#4CAF50";
      }
  ...
  return {
    ...
    progress,
    status,
    machineColor: getMachineColor('refinery') || "#8B6FBD", // Purple for refinery
    displayName: `Refinery #${liveMachine.id.split('-')[1] || ''}`,
    actions: {
      handlePauseResume,
...
export default useRefineryCard;