...
const useManufacturerCard = (machine) => {
  const {
    placedMachines,
    craftingQueue,
    pauseCrafting,
  ...
  const status = useMemo(() => {
    ...
    if (isProcessing && currentProcess) {
      if (currentProcess.status === "paused") {
        ...
        statusText = `Manufacturing: ${currentProcess.itemName}`;
        statusColor = "#4CAF50";
      }
  ...
  return {
    ...
    progress,
    status,
    machineColor: getMachineColor('manufacturer') || "#4D90CA", // Blue for manufacturer
    displayName: `Manufacturer #${liveMachine.id.split('-')[1] || ''}`,
    actions: {
      handlePauseResume,
...
export default useManufacturerCard;